...
class SMM_CPT_Unlimited {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( $this->_args['add_multiuploader'] ) {
            ...
            add_action( 'wp_ajax_smm_cptu_multiuploader', array( $this, 'post_multiuploader' ) );
        }
        ...
        if ( $this->_args['has_single'] ) {
            add_action( 'smm_loop', array( $this, 'single_template' ) );
        ...
        if ( ! empty( $this->_args['shortcode_name'] ) ) {
            ...
            add_filter( 'smm_shortcode_' . $this->_args['shortcode_name'] . '_icon', array( $this, 'shortcode_icon') );
            add_filter( 'smm-shortcode-plugin-init', array( $this, 'add_shortcode_to_box' ) );
        }
	...
	public function admin_sortable_assets() {
		...
		wp_enqueue_script( 'smm-cptu-sortable-posts', SMM_CORE_PLUGIN_URL . '/assets/js/smm-cptu-sortable-posts.js', array( 'jquery', 'jquery-ui-sortable' ), '1.0', true );
	}
	...
	public function init_menu_order( $post_types = array() ) {
		...
		foreach ( $post_types as $post_type ) {
			$count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM $wpdb->posts WHERE post_type = %s AND post_status IN ('publish', 'pending', 'draft', 'private', 'future') AND menu_order = 0",$post_id) );
	...
	public function sort_posts() {
		...
		parse_str( sanitize_text_field($_REQUEST['order']), $data );
    ...
    public function register_post_type() {
        $labels = array(
            ...
            'add_new'            => sprintf(/* translators: search here */ __( 'Add %s', 'smm-api' ), ucfirst( $this->_labels['singular'] ) ),
            'add_new_item'       => sprintf(/* translators: search here */ __( 'Add New %s', 'smm-api' ), ucfirst( $this->_labels['singular'] ) ),
            'edit_item'          => sprintf(/* translators: search here */ __( 'Edit %s', 'smm-api' ), ucfirst( $this->_labels['singular'] ) ),
            'new_item'           => sprintf(/* translators: search here */ __( 'New %s', 'smm-api' ), ucfirst( $this->_labels['singular'] ) ),
            'all_items'          => sprintf(/* translators: search here */ __( 'All %s', 'smm-api' ), ucfirst( $this->_labels['plural'] ) ),
            'view_item'          => sprintf(/* translators: search here */ __( 'View %s', 'smm-api' ), ucfirst( $this->_labels['singular'] ) ),
            'search_items'       => sprintf(/* translators: search here */ __( 'Search %s', 'smm-api' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found'          => sprintf(/* translators: search here */ __( 'No %s found', 'smm-api' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found_in_trash' => sprintf(/* translators: search here */ __( 'No %s found in Trash', 'smm-api' ), ucfirst( $this->_labels['plural'] ) ),
            'parent_item_colon'  => '',
            'menu_name'          => ucfirst( $this->_labels['menu'] )
        );

        $args = array(
            'labels'             => apply_filters( 'smm_' . $this->_name . '_labels', $labels ),
            'public'             => false,
        ...
        register_post_type( $this->_name, apply_filters( 'smm_' . $this->_name . '_args', $args ) );
    }
    ...
    protected function _cpt_args( $post ) {
        ...
        $args = apply_filters( 'smm_cptu_register_post_type_args', array(
    ...
    public function register_cptu_post_types() {
        ...
        foreach ( $post_types as $pt ) {
            ...
            $labels = array(
                ...
                'add_new'            => sprintf(/* translators: search here */ __( 'Add %s', 'smm-api' ), $label_singular ),
                'add_new_item'       => sprintf(/* translators: search here */ __( 'Add New %s', 'smm-api' ), $label_singular ),
                'edit_item'          => sprintf(/* translators: search here */ __( 'Edit %s', 'smm-api' ), $label_singular ),
                'new_item'           => sprintf(/* translators: search here */ __( 'New %s', 'smm-api' ), $label_singular ),
                'all_items'          => sprintf(/* translators: search here */ __( 'All %s', 'smm-api' ), $label_plural ),
                'view_item'          => sprintf(/* translators: search here */ __( 'View %s', 'smm-api' ), $label_singular ),
                'search_items'       => sprintf(/* translators: search here */ __( 'Search %s', 'smm-api' ), $label_plural ),
                'not_found'          => sprintf(/* translators: search here */ __( 'No %s found', 'smm-api' ), $label_plural ),
                'not_found_in_trash' => sprintf(/* translators: search here */ __( 'No %s found in Trash', 'smm-api' ), $label_plural ),
                'parent_item_colon'  => '',
                'menu_name'          => $title
            );

            $args = array(
                'labels'             => apply_filters( 'smm_' . $this->_prefix_cpt . $name . '_labels', $labels ),
                'public'             => true,
            ...
            // register post type
            $post_type = smm_avoid_duplicate( str_replace( '-', '_', substr( $this->_prefix_cpt . $name, 0, 16) ), $post_types );
            register_post_type( $post_type, apply_filters( 'smm_' . $this->_prefix_cpt . $name . '_args', $args, $pt ) );  // save the post type in post meta
            ...
            if ( $this->_args['has_taxonomy'] && ! empty( $taxonomy ) ) {
                ...
                $labels = array(
                    'name'              => sprintf(/* translators: search here */ _x( '%s Categories', 'taxonomy general name', 'smm-api' ), $label_singular ),
                    'singular_name'     => _x( 'Category', 'taxonomy singular name', 'smm-api' ),
                    'search_items'      => __( 'Search Categories', 'smm-api' ),
                    'all_items'         => __( 'All Categories', 'smm-api' ),
                    'parent_item'       => __( 'Parent Category', 'smm-api' ),
                    'parent_item_colon' => __( 'Parent Category:', 'smm-api' ),
                    'edit_item'         => __( 'Edit Category', 'smm-api' ),
                    'update_item'       => __( 'Update Category', 'smm-api' ),
                    'add_new_item'      => __( 'Add New Category', 'smm-api' ),
                    'new_item_name'     => __( 'New Category Name', 'smm-api' ),
                    'menu_name'         => __( 'Category', 'smm-api' ),
                );
        ...
        wp_cache_set( 'smm_cptu_post_types', $post_types );
    }
    ...
    public function add_cptu_menu_item() {
        ...
        // add new items
        if ( strpos( (string)$post_type, $this->_prefix_cpt ) !== false ) {
            global $wpdb;
            ...
            $submenu[ $item ][20] = array( sprintf(/* translators: search here */ __('Add %s', 'smm-api'), ucfirst( $label_singular ) ), 'edit_posts', 'post-new.php?post_type=' . $post_type );

            global $wp_taxonomies;
            $taxonomy = get_post_meta( $portfolio->ID, '_taxonomy', true );
            if ( isset( $wp_taxonomies[ $taxonomy ] ) ) {
                $submenu[ $item ][25] = array( __('Categories', 'smm-api'), 'edit_posts', 'edit-tags.php?taxonomy=' . $taxonomy . '&post_type=' . $post_type );
            }
        }
    ...
    public function frontend_assets() {
        ...
        if ( ! is_single() || ! isset( $post->post_type ) || ! $this->_is_valid( $post->post_type ) ) {
            ...
            foreach ( $posts as $post_id ) {
                ...
                foreach ( $this->layouts[ $layout ]['css'] as $asset ) {
                    ...
                    smm_enqueue_style( $asset['handle'], empty( $asset['src'] ) ? false : $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['media'] );
                }
        ...
        // load assets of single template
            ...
            foreach ( $this->layouts[ $layout ]['css'] as $asset ) {
                ...
                smm_enqueue_style( $asset['handle'], $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['media'] );
            }
        }
    ...
    public function add_metabox_cptu() {
        ...
        if ( $this->_args['manage_layouts'] ) {
            ...
            $layouts = apply_filters( 'smm_cptu_' . $this->_name . '_layout_values', $layouts );
        }

        $single_layouts = apply_filters( 'smm_cptu_' . $this->_name . '_single_layout_values', array() );

        $metabox_args = array(
            'label'    => sprintf(/* translators: search here */ __( '%s Settings', 'smm-api' ), $this->_labels['singular'] ),
            ...
            'tabs'     => array(
                'settings' => array(
                    ...
                    'fields' => apply_filters( 'smm_cptu_fields', array(
                        'type'      => array(
                            'label' => __( 'Type', 'smm-api' ),
                            'desc'  => sprintf(/* translators: search here */ __( 'Layout for this %s' , 'smm-api' ), strtolower( $this->_labels['singular'] ) ),
                            'type'  => 'select',
                            'options' => isset( $layouts ) ? $layouts : array(),
                            'std'   => '' ),

                        'rewrite'     => array(
                            'label' => __( 'Rewrite', 'smm-api' ),
                            'desc'  => __( 'Univocal identification name in the URL for each product (slug from post if empty)', 'smm-api' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_singular' => array(
                            'label' => __( 'Label in Singular', 'smm-api' ),
                            'desc'  => __( 'Set a label in singular (title of portfolio if empty)', 'smm-api' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_plural' => array(
                            'label' => __( 'Label in Plural', 'smm-api' ),
                            'desc'  => __( 'Set a label in plural (title of portfolio if empty)', 'smm-api' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy' => array(
                            'label' => __( 'Taxonomy', 'smm-api' ),
                            'desc'  => __( 'If you want to use categories in the portfolio, set a name for taxonomy. Name should be a slug (must not contain capital letters nor spaces) and must not be more than 32 characters long (database structure restriction).', 'smm-api' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy_rewrite' => array(
                            'label' => __( 'Taxonomy Rewrite', 'smm-api' ),
                            'desc'  => __( 'Set univocal name for each category page URL.', 'smm-api' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'single_layout' => array(
                            'label' => __( 'Single layout', 'smm-api' ),
                            'desc'  => __( 'Layout for single page of this portfolio', 'smm-api' ),
                            'type'  => 'select',
        ...
        if ( $this->_args['manage_layouts'] ) {
            ...
            foreach ( $this->layouts as $layout => $args ) {
                ...
                $metabox_args['tabs']['settings']['fields'][ $layout . '_title' ] = array(
                    'desc' => $args['name'] . ' ' . __( 'layout settings', 'smm-api' ),
                    'type' => 'title',
                    'deps' => array(
                        'ids' => '_type',
                        'values' => $layout
        ...
        $metabox = SMM_Metabox( $this->_name . '_cptu_settings' );
    ...
    public function add_metabox_item_fields() {
        ...
        $metabox = SMM_Metabox( $post_type . '_item_fields' );
        $metabox->init( $metabox_args );
    ...
    public function add_quick_links_metaboxes() {
        // CPTU
        add_meta_box( $this->_name . '_quick_links', __( 'Quick links', 'smm-api' ), array( $this, 'quick_links_cptu_inner' ), $this->_name, 'side', 'high' );
        ...
        foreach ( $post_types as $post ) {
            ...
            add_meta_box( $post->post_type . '_quick_links', __( 'Quick links', 'smm-api' ), array( $this, 'quick_links_cpt_inner' ), $post_type, 'side', 'high' );
        }
    }
    ...
    public function quick_links_cptu_inner( $post ) {
        extract( $this->_cpt_args( $post ) );
        ?>
        <a href="<?php echo esc_url( admin_url( 'edit.php?post_type=' . get_post_meta( $post->ID, '_post_type', true ) )) ?>"><?php printf(/* translators: search here */ esc_html__( 'View %s', 'smm-api' ), esc_html($label_plural) ) ?></a>
        <?php
    }
    ...
    public function quick_links_cpt_inner( $post ) {
        ...
        ?>
        <a href="<?php esc_url( admin_url( "post.php?post={$post->ID}&action=edit" ) )?>"><?php printf(/* translators: search here */ esc_html__( 'Edit %s', 'smm-api' ), esc_html($post->post_title )) ?></a>
        <?php
    }
    ...
    public function cptu_change_columns( $column, $post_id ) {
        ...
        switch ( $column ) {
            case 'actions' :
                esc_html( '<a href="' . admin_url( "post.php?post={$post_id}&action=edit" ) . '" class="button-secondary">' . sprintf(/* translators: search here */ __( 'Edit %s', 'smm-api' ), esc_html(ucfirst( $this->_labels['singular'] ) )) . '</a> ');
                esc_html( '<a href="' . admin_url( 'edit.php?post_type=' . get_post_meta( $post_id, '_post_type', true ) ) . '" class="button-secondary">' . sprintf(/* translators: search here */ __( 'View %s', 'smm-api' ), esc_html($label_plural) ) . '</a> ');
                break;
        }
    }
    ...
    public function add_shortcode( $atts, $content = null ) {
        ...
        if( function_exists( 'SMM_Portfolio' ) && method_exists( SMM_Portfolio(), 'is' ) && SMM_Portfolio()->is( $atts['name'] ) ){
            $atts['name'] = str_replace( SMM_Portfolio()->post_type_prefix, '', $atts['name'] );
        }
        ...
        extract( apply_filters( 'smm_cptu_frontend_vars', $atts, $this->_name ) );
        ...
        if ( $this->_args['manage_layouts'] && isset( $this->layouts[ $layout ]['js'] ) && ! empty( $this->layouts[ $layout ]['js'] ) ) {
            foreach ( $this->layouts[ $layout ]['js'] as $asset ) {
                ...
                smm_enqueue_script( $asset['handle'], $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['in_footer'] );
            }
        }
     ...
     *
     * @param $icon_url string Icone url found by smm_shortcode plugin
     *
     * @return string
     * @since 1.0.0
     * @author sam
    ...
    public function add_shortcode_to_box( $shortcodes ) {
        ...
        $args = array(
            $this->_args['shortcode_name'] => array(
                ...
                'description' => sprintf(/* translators: search here */ __( 'Show frontend of the %s', 'smm-api' ), esc_attr($this->_labels['main_name']) ),
                'tab'         => 'cpt',
                'create'      => false,
                'has_content' => false,
    ...
    protected function _is_valid( $post_type ) {
        return (bool)( strpos( (string)$post_type, $this->_args['post_type_prefix'] ) !== false );
    }
    ...
    */
    public function add_importer_required_post_type( $post ){
        ...
        if( strpos( (string)$post['post_type'], $this->_prefix_cpt ) === FALSE ){
            return $post;
        }
    ...
    */
    ...
    public function admin_assets() {
        wp_enqueue_media();
        wp_enqueue_script( 'smm-cptu', SMM_CORE_PLUGIN_URL . '/assets/js/smm-cpt-unlimited.js', array('jquery'), '1.0', true );
    }
        ...
        <script>
            (function($) {
                ...
                var button = $('<a />', {
                    ...
                    'data-uploader_title': '<?php printf(/* translators: search here */ esc_html__( 'Add %s from images', 'smm-api' ), esc_attr($label_plural) ) ?>',
/* translators: search here */                    'data-uploader_button_text': '<?php printf( esc_html__( 'Add %s', 'smm-api' ), esc_attr($label_plural) ) ?>'
                }).text('<?php esc_html_e( 'Upload multiple files', 'smm-api' ) ?>');
    ...
    public function post_multiuploader() {
        if ( ! isset( $_REQUEST['images'] ) || ! isset( $_REQUEST['post_type'] ) && $this->_is_valid( wc_clean($_REQUEST['post_type'] )) ) {
            return;
        }
        ...
        foreach ( $_REQUEST['images'] as $the ) {
            ...
            // Create post object
            $arg = array(
                'post_title' => sanitize_text_field($the['title']),
                'post_type'  => sanitize_text_field($_REQUEST['post_type'])
            );
            $post_id = wp_insert_post( $arg );

            set_post_thumbnail( $post_id, sanitize_text_field($the['id'] ));
...
//        }
        ...
        if ( $this->_args['manage_layouts'] ) {
            foreach ( $this->layouts[ 'single' ]['js'] as $asset ) {
                smm_enqueue_script( $asset['handle'], $this->locate_url( 'single' ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['in_footer'] );
            }
        }
        ...
        extract( apply_filters( 'smm_cptu_frontend_vars', array( 'name' => $post_name ), $this->_name ) );
    ...
    public function single_template_config() {
        ...
        if ( defined('DOING_AJAX') && DOING_AJAX && isset( $_REQUEST['post_id'] ) ) {
            $post = get_post( sanitize_text_field($_REQUEST['post_id']) );
        }
    ...
    public function archive_template() {
        ...
        remove_action( 'smm_content_loop', 'smm_content_loop', 10 );
        add_action( 'smm_content_loop', array( $this, 'archive_template_loop' ), 10 );
    ...
    public function archive_template_loop() {
        esc_html( $this->add_shortcode( array( 'name' => $GLOBALS['wp_query']->post->post_type ) ));
    }
