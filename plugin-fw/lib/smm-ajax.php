    ...
    class SMM_Ajax {
        ...
        private function __construct() {
            ...
            foreach ( $ajax_actions as $ajax_action ) {
                add_action( 'wp_ajax_smms_plugin_fw_' . $ajax_action, array( $this, $ajax_action ) );
                add_action( 'wp_ajax_nopriv_smms_plugin_fw_' . $ajax_action, array( $this, $ajax_action ) );
            }
        }
        ...
        public function json_search_posts( $request = false ) {
            ...
            $request = $request ? $request : wc_clean($_REQUEST);//phpcd:ignore
            ...
            foreach ( $args as $key => $default_value ) {
                if ( !empty( $request[ $key ] ) ) {
                    $args[ $key ] = sanitize_text_field($request[ $key ]);
                }
            }

            $show_id = isset( $request[ 'show_id' ] ) && wc_clean($request[ 'show_id' ]);
            ...
            if ( !empty( $posts ) ) {
                foreach ( $posts as $post_id ) {
                    ...
                    $found_posts[ $post_id ] = apply_filters( 'smms_plugin_fw_json_search_found_post_title', rawurldecode( $title ), $post_id, $request );
                }
            }
            $found_posts = apply_filters( 'smms_plugin_fw_json_search_found_posts', $found_posts, $request );
        ...
        public function json_search_products() {
            ...
            $request = apply_filters( 'smms_plugin_fw_json_search_products_request', $request );
        ...
        public function json_search_orders() {
            ...
            $term = apply_filters( 'smms_plugin_fw_json_search_order_number', $term );
		...
		", '%' . $term . '%' ) );
            ...
            if ( $query_orders ) {
                foreach ( $query_orders as $item ) {
                    $order_number              = apply_filters( 'smms_plugin_fw_order_number', '#' . $item->ID, $item->ID );
        ...
        public function json_search_terms() {
            ...
            $args = apply_filters( 'smms_plugin_fw_json_search_terms_default_args', $args, $request );
            ...
            $args = apply_filters( 'smms_plugin_fw_json_search_terms_args', $args, $request );
