(function ($) {
    ...
    const smms_icon = el('svg', {width: 22, height: 22},
    ...
    function create_shortcode(sc_args, props, callback) {
        ...
        if (typeof props.callback != 'undefined' && typeof $[props.callback] == 'function') {
            ...
            gutenberg_preview = '<span class="smms_block_' + block_id + '">' + sc + '</span>';

            if (callback == 'edit' && props.do_shortcode != false) {
                do_shortcode = (function (block_id) {
                    var ajax_call_date = null;
                    $(document).trigger( 'smms_plugin_fw_gutenberg_before_do_shortcode', [sc, block_id] );
                    $.ajax({
                        async: true,
                        url: smms_gutenberg_ajax.ajaxurl,
                        method: 'post',
                        data: {action: 'smms_plugin_fw_gutenberg_do_shortcode', shortcode: sc},
                        success: function (data) {
                            ajax_call_date = data;
                            if (ajax_call_date != '') {
                                $('.smms_block_' + block_id).html(ajax_call_date);
                                $(document).trigger( 'smms_plugin_fw_gutenberg_success_do_shortcode', [sc, block_id, ajax_call_date] );
                            }
                        }
                    });
                    $(document).trigger( 'smms_plugin_fw_gutenberg_after_do_shortcode', [sc, block_id, ajax_call_date] );
    ...
    $.each(smms_gutenberg, function ($block, $props) {
        registerBlockType("smms/" + $block, {
            title: $props.title,
            description: $props.description,
            category: $props.category,
            attributes: $props.attributes,
            icon: typeof $props.icon != 'undefined' ? $props.icon : smms_icon,
