...
(function ($) {
    ...
    $(act_page_option).insertAfter('#smm-post-setting .handlediv');
    $(act_page_option).insertAfter('#smm-page-setting .handlediv');


    $('#_active_page_options-container').on('click', function(){
        if( $('#_active_page_options').is(":checked") ){
            $('#smm-page-setting .inside .metaboxes-tab, #smm-post-setting .inside .metaboxes-tab').css( { 'opacity' : 1 , 'pointer-events' : 'auto' } );
        }else{
            $('#smm-page-setting .inside .metaboxes-tab, #smm-post-setting .inside .metaboxes-tab').css( { 'opacity' : 0.5 , 'pointer-events' : 'none' } );
        }
    }).click();
    ...
    function dependencies_handler ( id, deps, values, type ) {
        ...
        for ( j in types ) {
            ...
            if ( !result ) {
                switch ( current_type ) {
                    case 'disable':
                        $current_container.addClass( 'smms-disabled' );
                ...
                switch ( current_type ) {
                    case 'disable':
                        $current_container.removeClass( 'smms-disabled' );
