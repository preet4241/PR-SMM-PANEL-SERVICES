...
jQuery( function ( $ ) {
    ...
    function dependencies_handler( id, deps, values, type ) {
        ...
        var $current_field     = $( id ),
            $current_container = $( id + '-container' ).closest( 'tr' ); // container for SMM Plugin Panel

        if ( $current_container.length < 1 ) {
            // container for SMM Plugin Panel WooCommerce
            $current_container = $current_field.closest( '.smms-plugin-fw-panel-wc-row' );
        }
    ...
    $( '.smms-plugin-fw-panel .woo-nav-tab-wrapper' ).removeClass( 'woo-nav-tab-wrapper' ).addClass( 'smms-nav-tab-wrapper' );
