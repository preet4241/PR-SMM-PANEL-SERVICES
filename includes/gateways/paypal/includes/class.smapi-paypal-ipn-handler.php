...
/**
 * Implements SMAPI_PayPal_IPN_Handler Class
 *
 * @class   SMAPI_PayPal_IPN_Handler
 * @package SMMS WooCommerce Subscription
 * @since   1.0.1
 * @author  SMMS
 */
if ( !class_exists( 'SMAPI_PayPal_IPN_Handler' ) ) {

	class SMAPI_PayPal_IPN_Handler extends WC_Gateway_Paypal_IPN_Handler {
		...
		protected function process_paypal_request( $order, $posted ) {
			...
			WC_Gateway_Paypal::log( 'SMAPI - Validate currency OK' );

			if ( isset( $posted['receiver_email'] ) ) {
				$this->validate_receiver_email( $order, $posted['receiver_email'] );
			}

			WC_Gateway_Paypal::log( 'SMAPI - Receiver Email OK' );
		...
		protected function paypal_ipn_request( $ipn_args ) {
			...
			WC_Gateway_Paypal::log( 'SMAP - Paypal IPN Request Start' );
			...
			if ( $order->get_order_key() != $order_info['order_key'] ) {
				WC_Gateway_Paypal::log( 'SMAP - Order keys do not match' );
				return;
			}

			//check if the transaction has been processed
			$order_transaction_ids = smm_get_prop( $order, '_paypal_transaction_ids');
			...
			if ( $order_transactions ) {
				...
				WC_Gateway_Paypal::log( 'SMAP - Transaction ID already processed' );
				return;
			}
			...
			if ( empty( $subscriptions ) ) {
				WC_Gateway_Paypal::log( 'SMAP - IPN subscription payment error - ' . $order_id . ' haven\'t subscriptions' );
				return;
			} else {
				WC_Gateway_Paypal::log( 'SMAP - Subscription ' . print_r( $subscriptions, true ) );
			}
			...
			switch ( $ipn_args['txn_type'] ) {
				case 'subscr_signup':
					...
					$order->add_order_note( __( 'IPN subscription started', 'smm-api' ) );
				...
				case 'subscr_payment':
					if ( 'completed' == strtolower( $ipn_args['payment_status'] ) ) {
						...
						foreach ( $subscriptions as $subscription_id ) {
							...
							if ( $subscription->status == 'pending' || ( ! $last_order && $order->has_status( $valid_order_statuses ) ) ) {
								...
								smm_save_prop( $order, $args );
								$order->add_order_note( __( 'IPN subscription payment completed.', 'smm-api' ) );
								$order->payment_complete( $ipn_args['txn_id'] );
								...
								smm_save_prop( $last_order, $args, false, true );

								$last_order->add_order_note( __( 'IPN subscription payment completed.', 'smm-api' ) );
								$last_order->payment_complete( $ipn_args['txn_id'] );

							} else {

								//if the renew_order is not created try to create it
								$new_order_id = SMAPI_Subscription_Order()->renew_order( $subscription->id );
								...
								smm_save_prop( $new_order_id, $args, false, true );

								$new_order->add_order_note( __( 'IPN subscription payment completed.', 'smm-api' ) );
					...
					if ( isset( $ipn_args['subscr_id'] ) && 'pending' == strtolower( $ipn_args['payment_status'] ) && 'echeck' == strtolower( $ipn_args['payment_type'] ) ) {
						...
						foreach ( $subscriptions as $subscription_id ) {
							...
							if ( $subscription->status == 'pending' || ( ! $last_order && $order->has_status( $valid_order_statuses ) ) ) {
								...
								$order->update_status( 'on-hold', __( 'Paypal echeck payment', 'smm-api' ) );
								smm_save_prop( $order, 'Payment type', $ipn_args['payment_type'], false, true );
								wc_reduce_stock_levels( $order_id );
								WC()->cart->empty_cart();

							} elseif ( $last_order ) {
								//renew order
								$last_order->add_order_note( __( 'SMAP - IPN Pending payment for echeck payment type', 'smm-api' ) );
								...
								$last_order->add_order_note( __( 'SMAP - Subscription has been suspendend because in pending payment for echeck payment type', 'smm-api' ) );
								...
								$new_order = wc_get_order( $new_order_id );
								$new_order->add_order_note( __( 'SMAP - IPN Pending payment for echeck payment type', 'smm-api' ) );
							}
						}
					...
					if ( isset( $ipn_args['subscr_id'] ) && 'failed' == strtolower( $ipn_args['payment_status'] ) ) {
						if ( isset( $ipn_args['subscr_id'] ) ) {
							...
							$order_sub_id  = smm_get_prop( $order, 'Subscriber ID');

							if ( $paypal_sub_id != $order_sub_id ) {
								WC_Gateway_Paypal::log( 'SMAP - IPN subscription failed request ignored - new PayPal Profile ID linked to this subscription, for order ' . $order_id );
							} else {
								$subscriptions = smm_get_prop( $order, 'subscriptions' );

								if ( empty( $subscriptions ) ) {
									WC_Gateway_Paypal::log( 'SMAP - IPN subscription failed payment request ignored - order ' . $order_id . ' doesn\'t not subscriptions' );
								}
								...
								foreach ( $subscriptions as $subscription_id ) {
									...
									$subscription = smapi_get_subscription( $subscription_id );
									...
									if ( $subscription->status == 'pending' || ( ! $last_order && $order->has_status( $valid_order_statuses ) ) ) {
										...
										$last_order->add_order_note( __( 'SMAP - IPN Failed payment', 'smm-api' ) );
									} else {
										//if the renew_order is not created try to create it
										$new_order_id = SMAPI_Subscription_Order()->renew_order( $subscription->id );
										if( ! $new_order_id ){
										...
										$new_order = wc_get_order( $new_order_id );
										$new_order->add_order_note( __( 'SMAP - IPN Failed payment', 'smm-api' ) );
									}
									...
									if ( isset( $ipn_args['retry_at'] ) ) {
										smm_save_prop( $order, 'next_payment_attempt', strtotime( $ipn_args['retry_at'] ) , false, true);
									}

									if ( get_option( 'smapi_suspend_for_failed_recurring_payment' ) == 'yes' ) {
										$subscription->update_status( 'suspended', 'paypal' );
									}

									$order->add_order_note( __( 'SMAP - IPN Failed payment', 'smm-api' ) );

									// Subscription Cancellation Completed

									WC_Gateway_Paypal::log( 'SMAP -IPN Failed payment' . $order_id );
				...
				case 'recurring_payment_skipped':
					if ( isset( $ipn_args['recurring_payment_id'] ) ) {
						...
						if ( $paypal_sub_id != $order_sub_id ) {
							WC_Gateway_Paypal::log( 'SMAP - IPN subscription failed payment - new PayPal Profile ID linked to this subscription, for order ' . $order_id );
						} else {
							$subscriptions = smm_get_prop( $order, 'subscriptions' );
				...
				case 'recurring_payment_suspended_due_to_max_failed_payment':
					if( isset($ipn_args['recurring_payment_id'])){
						...
						if ( $paypal_sub_id != $order_sub_id ) {
							...
							if ( empty( $subscriptions ) ) {
								WC_Gateway_Paypal::log( 'SMAP - IPN subscription cancellation for failed request ignored - order ' . $order_id . ' doesn\'t not subscriptions' );
							}
							...
							foreach ( $subscriptions as $subscription_id ) {
								...
								$max_failed_attemps = smapi_get_max_failed_attemps_list();

								if ( $failed_attemp >= $max_failed_attemps['paypal'] - 1 ) {
									$subscription->cancel( false );
									$order->add_order_note( __( 'SMAP - Subscription cancelled max failed attemps: recurring_payment_suspended_due_to_max_failed_payment', 'smm-api' ) );
									// Subscription Cancellation Completed
									WC_Gateway_Paypal::log( 'SMAP -Subscription cancelled max failed attempts: recurring_payment_suspended_due_to_max_failed_payment' . $order_id );
								} else {
									$subscription->update_status( 'suspended', 'paypal' );
									$last_order    = false;
									$pending_order = $subscription->renew_order;
									...
									if ( $last_order ) {
										$last_order->add_order_note( __( 'SMAP - IPN message: recurring_payment_suspended_due_to_max_failed_payment', 'smm-api' ) );
									} else {
										$order->add_order_note( __( 'SMAP - IPN message: recurring_payment_suspended_due_to_max_failed_payment', 'smm-api' ) );
									}
								}
				...
				case 'subscr_cancel':
					...
					if ( $paypal_sub_id != $order_sub_id ) {
						WC_Gateway_Paypal::log( 'SMAP - IPN subscription cancellation request ignored - new PayPal Profile ID linked to this subscription, for order ' . $order_id );
					} else {
						$subscriptions = smm_get_prop( $order, 'subscriptions' );

						if ( empty( $subscriptions ) ) {
							WC_Gateway_Paypal::log( 'SMAP - IPN subscription cancellation request ignored - order ' . $order_id . ' doesn\'t not subscriptions' );
						}
						...
						foreach ( $subscriptions as $subscription_id ) {
							...
							$subscription = new SMAPI_Subscription( $subscription_id );
							$subscription->cancel( false );
							$order->add_order_note( __( 'SMAP - IPN subscription cancelled for the order.', 'smm-api' ) );

							// Subscription Cancellation Completed
							WC_Gateway_Paypal::log( 'SMAP -IPN subscription cancelled for order ' . $order_id );
		...
		protected function is_a_valid_transaction( $transaction_ids, $ipn_args  ) {
			...
			if ( isset( $ipn_args['txn_id'] ) ) {
				...
				if ( empty( $transaction_ids ) || ! in_array( $transaction_id, $transaction_ids ) ) {
					...
					if ( $this->debug ) {
						$this->wclog->add( 'paypal', 'SMAP - Subscription IPN Error: IPN ' . $transaction_id . ' message has already been correctly handled.' );
					}
				...
				if ( empty( $transaction_ids ) || ! in_array( $track_id, $transaction_ids ) ) {
					...
					if ( $this->debug ) {
						$this->wclog->add( 'paypal', 'SMAP - Subscription IPN Error: IPN ' . $track_id . ' message has already been correctly handled.' );
					}
					return false;
				}
			}
		...
		protected function validate_amount( $order, $amount ) {
			if ( number_format( $order->get_total(), 2, '.', '' ) != number_format( $amount, 2, '.', '' ) ) {
				...
				$order->update_status(/* translators: %s: amount */ 'on-hold', sprintf( __( 'Validation error: PayPal amounts do not match (gross %s).', 'smm-api' ), $amount ) );
				exit;
			}
		}
		...
		protected function payment_status_refunded( $order, $posted ) {
			...
			if ( floatval( $order->get_total() ) ==  $posted['mc_gross'] * -1 ) {
				...
				$order->update_status( 'refunded', sprintf( __( 'Payment %s via IPN.',  'smm-api' ), strtolower( $posted['payment_status'] ) ) );

				$this->send_ipn_email_notification(
				/* translators: %s: order link. */
					sprintf( __( 'Payment for order %s refunded', 'smm-api' ), '<a class="link" href="' . esc_url( $order->get_edit_order_url() ) . '">' . $order->get_order_number() . '</a>' ),
					/* translators: %1$s: order ID, %2$s: reason code. */
					sprintf( __( 'Order #%1$s has been marked as refunded - PayPal reason code: %2$s',  'smm-api' ), $order->get_order_number(), $posted['reason_code'] )
				);
			}
		}
